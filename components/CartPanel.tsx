    ...
    <>
      ...
      >
          ...
          {mounted && cart.map((item: CartItem) => (
            <div key={item.nombre} className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => removeFromCart(item.nombre)} className="w-7 h-7 rounded-full bg-[#128708] text-white flex items-center justify-center text-base font-bold hover:opacity-80 transition">−</button>
                <span className="text-sm font-bold text-[#162B45] w-4 text-center">{item.cantidad}</span>
                <button onClick={() => addToCart(item)} className="w-7 h-7 rounded-full bg-[#128708] text-white flex items-center justify-center text-base font-bold hover:opacity-80 transition">+</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="p-4 border-t space-y-3">
          {mounted && cart.length > 0 && (
            <button onClick={clearCart} className="w-full text-sm text-gray-400 hover:text-red-500 transition">
              Vaciar carrito
            </button>
          )}
